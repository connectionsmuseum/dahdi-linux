...
static int initialize_channel(struct dahdi_chan *chan)
{
	...
	if (chan->sig == DAHDI_SIG_RPT) {
		...
		chan->pulsemaketime = 40;
		chan->pulsebreaktime = 40;
		chan->pulseaftertime = 40;
	} else {
...
static int ioctl_dahdi_dial(struct dahdi_chan *chan, unsigned long data) // XXX SA need to get to here to dial
{
...
static int dahdi_chan_ioctl(struct file *file, unsigned int cmd, unsigned long data)
{
	...
	case DAHDI_HOOK:
		...
		if (chan->span->flags & DAHDI_FLAG_RBS) {
			...
			case DAHDI_WINK:
				spin_lock_irqsave(&chan->lock, flags);
				/* If this is an RPT call, Asterisk should send a wink after it is satisfied
				   with selections (after it has stored FU). The wink comes here via ioctl,
				   and we advance our sequence switch. Next time the real sender goes closes
				   the loop (OFF HOOK), commutator() is called, and handles the Incoming Advance
				   and signaling cleanup. CD-25013-01 Section 11.8
				*/
				if (chan->sig == DAHDI_SIG_RPT) {
...
static inline void __rbs_otimer_expire(struct dahdi_chan *chan)
{
	...
	case DAHDI_TXSTATE_FLASH:
		dahdi_rbs_sethook(chan, DAHDI_TXSIG_OFFHOOK, DAHDI_TXSTATE_OFFHOOK, 0);
		if (chan->sig != DAHDI_SIG_RPO) {
			if (chan->file && (chan->file->f_flags & O_NONBLOCK)) {		// XXX SA stop sending hook complete on every RP
				__qevent(chan, DAHDI_EVENT_HOOKCOMPLETE);
			}
		} else {
			wake_up_interruptible(&chan->waitq);
	...
	case DAHDI_TXSTATE_AFTERSTART:
		if (chan->sig == DAHDI_SIG_RPT) {
			/* We got here from commutator() after an 80ms delay. This was to allow relays to settle.
			   Now, send our first pulse, and return to RPBREAK when pulse is complete */
			dahdi_rbs_sethook(chan, DAHDI_TXSIG_PULSE, DAHDI_TXSTATE_RPBREAK, chan->pulsebreaktime);
			// module_printk(KERN_NOTICE, "STATE AFTERSTART (%x)\n", chan->txsig);
		} else {
			dahdi_rbs_sethook(chan, DAHDI_TXSIG_OFFHOOK, DAHDI_TXSTATE_OFFHOOK, 0);
		}
		if (chan->file && (chan->file->f_flags & O_NONBLOCK))
    ...
    case DAHDI_TXSTATE_RPMAKE:
		/* Got here from RPBREAK.
		   Also handles telltale!
	...
	case DAHDI_TXSTATE_RPAFTER:
		/* By the time we get here, commutator() has already sent a PULSE to discharge the cable and the STP
		 relay in the sub sender. This has no other effect. (CD-25013-01 11.813) */
		module_printk(KERN_NOTICE, "STATE RP-AFTER (%x), instance %d\n", chan->txsig, chan->seqswitch);
		...
		case DAHDI_RXSIG_PULSE:
			...
			if ((chan->rpdebtimer > 18700)) {
				...
				module_printk(KERN_NOTICE, "Pulse: %d, SELECTION: %d,  i-%d\n",
						chan->pulsecount-1, chan->selidx, chan->rpdebtimer);

				if (chan->pulsecount == chan->selections[chan->selidx]+1) {
